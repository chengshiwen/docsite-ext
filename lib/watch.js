...
const mdWatchDirs = (() => {
  ...
  pages.forEach(page => {
    ...
    if (fs.statSync(path.join(CWD, './src/pages', page)).isDirectory()) {
      dirs.push(path.join(CWD, page));
    }
  });
  return dirs;
})();
const mdWatcher = chokidar.watch(mdWatchDirs, {
  ignoreInitial: true, // 默认为false，会在初始化时一直触发add和addDir事件
});
const ejsWatcher = chokidar.watch([path.join(CWD, './template.ejs'), path.join(CWD, './redirect.ejs')], {
  ignoreInitial: true, // 默认为false，会在初始化时一直触发add和addDir事件
});
// md文档监听
mdWatcher
