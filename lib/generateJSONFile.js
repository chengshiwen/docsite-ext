...
const generate = (env, cwd, type, lang, mdData) => {
  ...
  if (fs.existsSync(path.join(cwd, type, lang)) && fs.statSync(path.join(cwd, type, lang)).isDirectory()) {
    parse(path.join(cwd, type, lang));
  }
};

const generatePageJSONFile = (env, cwd, page) => {
  if (!(fs.existsSync(path.join(cwd, page)) && fs.statSync(path.join(cwd, page)).isDirectory())) return;
  const data = {
    'en-us': [],
    'zh-cn': [],
  };
