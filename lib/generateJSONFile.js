  ...
  const parse = dir => {
    ...
    files.forEach(file => {
      ...
      if (stat.isFile() && (extension === '.md' || extension === '.markdown')) {
        ...
        // 同步生成HTML，避免再遍历一遍目录
        // 生成文档
        if (type === 'docs' && fs.existsSync(path.join(cwd, './src/pages/documentation'))) {
          renderHTMLFile(cwd, lang, 'documentation');
        } else if (type === 'blog' && fs.existsSync(path.join(cwd, './src/pages/blogDetail'))) {
          renderHTMLFile(cwd, lang, 'blogDetail');
        } else if (type === 'community' && fs.existsSync(path.join(cwd, './src/pages/community'))) {
          renderHTMLFile(cwd, lang, 'community');
        }
...
const generateJSONFile = (env, cwd) => {
  ...
  generate(env, cwd, 'community', 'zh-cn', communityData);
  generate(env, cwd, 'community', 'en-us', communityData);
