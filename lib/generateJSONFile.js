    ...
    files.forEach(file => {
      ...
      if (stat.isFile() && (extension === '.md' || extension === '.markdown')) {
        ...
        if (
          ...
          type === 'community' &&
          fs.existsSync(path.join(cwd, './src/pages/community'))
        ) {
          const Community = require(path.join(cwd, './src/pages/community')).default;
          ejs.renderFile(
            path.join(cwd, './template.ejs'),
            {
              title: result.meta.title || fileInfo.name,
              ...
              page: 'community',
              __html: ReactDOMServer.renderToString(
                React.createElement(Community, { lang, __html: result.__html, meta: result.meta }, null)
              ),
            },
            (err, str) => {
              if (err) {
