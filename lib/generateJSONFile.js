  ...
  const parse = dir => {
    ...
    files.forEach(file => {
      ...
      if (stat.isFile() && (extension === '.md' || extension === '.markdown')) {
        ...
        const targetPath = `${splitArr[0]}${dist}${path.sep}${lang}${path.sep}${type}${splitArr[1]}`;
        ...
        const renderHTMLFile = (cwd, lang, page) => {
          ...
          ejs.renderFile(
            path.join(cwd, './template.ejs'),
            {
              ...
              pageCssPath: getManifestValue(page + '.md.css'),
              vendorJsPath: getManifestValue('vendor.js'),
              pageJsPath: getManifestValue(page + '.md.js'),
...
const generateJSONFile = (env, cwd) => {
  ...
  pages.forEach(page => {
    if (page === 'home') return;
    if (fs.statSync(path.join(cwd, './src/pages', page)).isDirectory()) {
      generatePageJSONFile(env, cwd, page);
    }
  });
};
