...
const renderHTMLFile = (cwd, lang, Page, page) => {
  let fileDir = page === 'home' ? path.join(cwd, lang) : path.join(cwd, lang, page);
  ...
  ejs.renderFile(
    ...
    (err, str) => {
      ...
      let filePath = page === 'home' ? path.join(cwd, lang, 'index.html') : path.join(cwd, lang, page, 'index.html');
      fs.writeFileSync(filePath, str, 'utf8');
    }
  );
};

const renderHTMLFileForLangs = (cwd, page) => {
    // 文件夹并且下面有index.jsx文件
    if (fs.existsSync(path.join(cwd, './src/pages', page, './index.jsx'))) {
      ...
      // 生成英文版
      renderHTMLFile(cwd, 'en-us', Page, page);
      // 生成中文版
      renderHTMLFile(cwd, 'zh-cn', Page, page);
    }
}
