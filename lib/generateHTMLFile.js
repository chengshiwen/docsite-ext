...
const getManifestValue = (key) => {
  if (typeof key !== 'string' || !key) return key;
  if (Object.keys(manifest).length) return manifest[key];
  if (key === 'vendor.css' || key === 'vendor.js') return null;
  return key;
};
...
const renderHTMLFile = (cwd, lang, Page, page, subPage = '') => {
  ...
  ejs.renderFile(
    path.join(cwd, './template.ejs'),
    {
      ...
      vendorCssPath: getManifestValue('vendor.css'),
      pageCssPath: getManifestValue(page + '.css'),
      vendorJsPath: getManifestValue('vendor.js'),
      pageJsPath: getManifestValue(page + '.js'),
...
const generateDocsLatest = (cwd, lang) => {
  const targetPath = path.join(cwd, dist, lang, 'docs');
  if (siteConfig.docsLatest && !siteConfig.docsLatest.includes(path.sep) && fs.existsSync(path.join(targetPath, siteConfig.docsLatest))) {
    shell.cd(targetPath);
    fs.removeSync('latest');
    fs.ensureSymlinkSync(siteConfig.docsLatest, 'latest', 'dir');
    shell.cd(cwd);
  }
};
...
const generateHTMLFile = (env, cwd) => {
  ...
  // 生成docs latest
  generateDocsLatest(cwd, 'en-us');
  generateDocsLatest(cwd, 'zh-cn');
};
