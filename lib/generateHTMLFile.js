...
const renderHTMLFile = (cwd, lang, Page, page, subPage='') => {
  let fileDir = page === 'home' ? path.join(cwd, lang) : path.join(cwd, lang, page, subPage);
  ...
  ejs.renderFile(
    ...
    (err, str) => {
      ...
      let filePath = page === 'home' ? path.join(cwd, lang, 'index.html') : path.join(cwd, lang, page, subPage, 'index.html');
...
const renderHTMLFileForLangs = (cwd, page, subPage='') => {
    ...
    if (fs.statSync(targetPath).isDirectory() && fs.existsSync(path.join(targetPath, 'index.jsx'))) {
